 // Variables globales optimizadas
    ...
    function setupEventListeners() {
      try {
        ...
        // v3.3.12: Análisis visual — histograma, paleta y auto-balance.
        const histogramBtn = document.getElementById('histogram-btn');
        if (histogramBtn) {
          histogramBtn.addEventListener('click', showHistogram);
        }
    ...
    function showHistogram() {
      ...
      // Normalización: usar el máximo de los 4 canales para que la
      // escala vertical sea consistente.
      let max = 0;
      for (let i = 0; i < 256; i++) {
        if (r[i] > max) max = r[i];
        if (g[i] > max) max = g[i];
      ...
      // Pintamos en orden y con opacidad para que los canales se mezclen.
    ...
    function _extractDominantColors(imageData, count) {
      // Cuantización en buckets de 32×32×32 (~32K colores totales).
      const data = imageData.data;
      const buckets = new Map();
      // Sample cada 4 píxeles (4 bytes × 4 = 16) para no recorrer millones
      // en imágenes grandes; el resultado es prácticamente idéntico.
      const stride = 16;
      for (let i = 0; i < data.length; i += stride) {
        if (data[i + 3] === 0) continue;
        const r = data[i] >> 5;     // 8 niveles
        const g = data[i + 1] >> 5; // 8 niveles
        const b = data[i + 2] >> 5; // 8 niveles
        const key = (r << 6) | (g << 3) | b;
    ...
    function autoBalanceImage() {
      ...
      // Construir histograma de luminosidad ignorando alpha=0.
      for (let i = 0; i < data.length; i += 4) {
        if (data[i + 3] === 0) continue;
      ...
      // Calcular percentiles 1% y 99% de luminosidad para evitar que
      // unos pocos píxeles atípicos arruinen el contraste global.
      ...
      // Construir LUT que mapea linealmente [lo, hi] -> [0, 255].
      ...
      // Aplicar la LUT a los 3 canales por separado para preservar color.
      for (let i = 0; i < data.length; i += 4) {
        if (data[i + 3] === 0) continue;
        data[i]     = lut[data[i]];
      ...
      // Guardar en historial para que sea undo-able.
