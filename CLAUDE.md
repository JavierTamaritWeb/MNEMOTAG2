...
**Current version: v3.3.13** (feature release: editor de curvas y niveles estilo Photoshop — modal con canvas interactivo 280×280 donde el usuario añade/mueve/elimina puntos de control por canal (RGB combinado, R, G y B individuales), curva renderizada en tiempo real con interpolación lineal segmentada, cuadrícula 4×4, línea diagonal de identidad, y al pulsar "Aplicar" se construyen 4 LUTs `Uint8ClampedArray(256)` y se aplican a `ctx.getImageData()` con composición Photoshop-style: primero la LUT por canal (R/G/B individuales), luego la LUT RGB combinada encima. Persiste en historial via `historyManager.saveState()`). Sources kept in sync: `index.html` `<title>`, `README.md` badge + NOVEDADES heading, `CHANGELOG.md` header + footer + topmost entry, and `docs/INDICE_DOCUMENTACION.md`. Note: `docs/RESUMEN_VERSIONES.md` is a hand-curated historical table that still marks `v3.1.4` as "Actual" — it has not been updated in the bump and would need a dedicated entry with narrative. `git log` remains the authoritative source for the actual commit version.
