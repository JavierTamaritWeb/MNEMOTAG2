...
**Current version: v3.3.15** (feature release: soporte HEIC/HEIF — los formatos por defecto de iPhone. Carga desde CDN de `heic2any@0.0.4`. `handleFile` ahora es async y, si detecta una imagen HEIC/HEIF (por MIME `image/heic|heif` o por extensión `.heic|.heif` en el nombre), llama a `heic2any({blob:file, toType:'image/jpeg', quality:0.92})` ANTES de validar, envuelve el blob resultante en un `File` con extensión `.jpg` y deja que el flujo normal de carga continúe. `SecurityManager.validateImageFile` añade `image/heic` e `image/heif` a `allowedTypes` SOLO si `typeof heic2any !== 'undefined'`. Sin librería disponible, error claro al usuario sin romper nada más). Sources kept in sync: `index.html` `<title>`, `README.md` badge + NOVEDADES heading, `CHANGELOG.md` header + footer + topmost entry, and `docs/INDICE_DOCUMENTACION.md`. Note: `docs/RESUMEN_VERSIONES.md` is a hand-curated historical table that still marks `v3.1.4` as "Actual" — it has not been updated in the bump and would need a dedicated entry with narrative. `git log` remains the authoritative source for the actual commit version.
