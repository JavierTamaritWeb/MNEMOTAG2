...
**Current version: v3.3.14** (feature release: panel de historial visual con thumbnails clicables. Botón "Historial" junto a Deshacer/Rehacer abre un panel `<div id="history-panel">` debajo del canvas que muestra los snapshots del historial como mini-cards con la imagen completa (`object-fit: contain`) y un timestamp. Click sobre un thumbnail llama a `historyManager.jumpToState(index)` para saltar directamente a ese estado. El thumbnail actual se resalta con borde azul. El panel se re-renderiza automáticamente vía `window.renderHistoryPanel` (hook llamado desde `historyManager.updateUndoRedoButtons` para evitar acoplamiento entre managers). Cero dependencias nuevas). Sources kept in sync: `index.html` `<title>`, `README.md` badge + NOVEDADES heading, `CHANGELOG.md` header + footer + topmost entry, and `docs/INDICE_DOCUMENTACION.md`. Note: `docs/RESUMEN_VERSIONES.md` is a hand-curated historical table that still marks `v3.1.4` as "Actual" — it has not been updated in the bump and would need a dedicated entry with narrative. `git log` remains the authoritative source for the actual commit version.
