...
**Current version: v3.4.0** (umbrella release). Consolida bajo una sola versión mayor las 8 features publicadas en v3.3.11–v3.3.18 (paste portapapeles + export multi-size ZIP, histograma + paleta + auto-balance, curvas y niveles estilo Photoshop, historial visual con thumbnails, soporte HEIC/HEIF via `heic2any`, PWA con Service Worker, parser ISOBMFF defensivo para AVIF, eliminar fondo con IA lazy load). Añade ~164 líneas de CSS al final de `css/styles.css` con selectores por ID para los 5 botones nuevos (`#auto-balance-btn` ámbar, `#curves-btn` morado, `#remove-bg-btn` cian, `#download-multisize-btn` outlined con dark mode, `#history-toggle-btn` índigo), con focus accesible y wrap del contenedor Undo/Redo/Historial. Cache-bust de `styles.css` vía query string (`?v=20260409a`) y bump del Service Worker a `mnemotag-v3.3.19-css-fix` para invalidar caches antiguos en PWAs ya instaladas. Copyright bumped to 2026 in footer + `metaCopyright` placeholder + README.md. Push unblocked by moving `.github/workflows/README.md` out of the workflows directory (it wasn't a workflow — just the docs for them). Sources kept in sync: `index.html` `<title>`, `README.md` badge + NOVEDADES heading + v3.3.18 entry restaurada, `CHANGELOG.md` header "v3.4" + footer + entrada [3.4.0] propia al tope, `docs/INDICE_DOCUMENTACION.md` header y pie, `docs/README.md` y `docs/RESUMEN_VERSIONES.md` actualizados para marcar v3.4.0 como Actual. Tests sin cambios: 142/142 Node + 44/44 binarios. `git log` remains the authoritative source for the actual commit version.
