...
**Current version: v3.3.8** (patch release: añade `tests/binary-validation.js`, un script Node que verifica byte por byte la manipulación binaria de chunks PNG `eXIf` y WebP RIFF/VP8X. 36 aserciones contra archivos PNG y WebP sintetizados a mano. Complementa `tests/run-in-node.js` para detectar regresiones en el código de bajo nivel sin necesidad de browser). Sources kept in sync: `index.html` `<title>`, `README.md` badge + NOVEDADES heading, `CHANGELOG.md` header + footer + topmost entry, and `docs/INDICE_DOCUMENTACION.md`. Note: `docs/RESUMEN_VERSIONES.md` is a hand-curated historical table that still marks `v3.1.4` as "Actual" — it has not been updated in the bump and would need a dedicated entry with narrative. `git log` remains the authoritative source for the actual commit version.
