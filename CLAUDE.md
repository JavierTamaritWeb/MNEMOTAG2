...
**Current version: v3.3.9** (patch release: añade workflows de GitHub Actions para CI/CD + deploy automático a GitHub Pages. NO toca el código de la app — sigue siendo 100% static, sin npm. Solo añade 3 archivos en `.github/workflows/`. La primera ejecución del workflow de deploy requiere una acción manual del usuario en `Settings → Pages → Source = GitHub Actions`). Sources kept in sync: `index.html` `<title>`, `README.md` badge + NOVEDADES heading, `CHANGELOG.md` header + footer + topmost entry, and `docs/INDICE_DOCUMENTACION.md`. Note: `docs/RESUMEN_VERSIONES.md` is a hand-curated historical table that still marks `v3.1.4` as "Actual" — it has not been updated in the bump and would need a dedicated entry with narrative. `git log` remains the authoritative source for the actual commit version.
