...
- **CI/CD con GitHub Actions** (desde v3.3.9): hay dos workflows en `.github/workflows/`:
  ...
  - `deploy.yml`: tras cada push a `main`, re-corre los tests como defensa en profundidad y despliega el repo entero a **GitHub Pages** (`https://javiertamaritweb.github.io/IMGCRAFT2/`). Sin build step. La primera ejecución **falla** hasta que se active Pages a mano en `Settings → Pages → Source = GitHub Actions`. Documentado en `.github/workflows/README.md`.
...
Sources kept in sync: `index.html` `<title>`, `README.md` badge + NOVEDADES reescrita como resumen v3.4.1→v3.4.15, `CHANGELOG.md` con entradas individuales [3.4.1]–[3.4.15] + footer, `docs/INDICE_DOCUMENTACION.md`, `docs/RESUMEN_VERSIONES.md`, `docs/README.md`. Service Worker `CACHE_VERSION = 'imgcraft-v3.4.15'`.
