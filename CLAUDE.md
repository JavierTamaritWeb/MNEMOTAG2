...
**Current version: v3.3.18** (feature release — Eliminar fondo con IA, lazy load total). Botón "Eliminar fondo (IA)" en la sección de filtros que dispara una descarga ÚNICA del modelo `@imgly/background-removal@1.4.5` (~10-15 MB) la primera vez que el usuario lo pulsa, vía `await import('https://cdn.jsdelivr.net/npm/@imgly/background-removal@1.4.5/+esm')`. El módulo se cachea en `_bgRemovalModule` para reutilizarse en clicks posteriores sin re-descargar. Toast informativo del tamaño del modelo antes de descargar para gestionar expectativas. Tras procesar, el resultado se carga al canvas y se persiste en `historyManager.saveState()` para que sea undo-able. Degradación elegante completa: si la librería falla al cargar (offline, CORS, error de red), error claro al usuario sin romper nada más. **Cero impacto en el peso inicial de la app** — el módulo solo se descarga si el usuario hace click. Esta es la última versión del bloque de 8 commits planificado. Sources kept in sync: `index.html` `<title>`, `README.md` badge + NOVEDADES heading, `CHANGELOG.md` header + footer + topmost entry, and `docs/INDICE_DOCUMENTACION.md`. Note: `docs/RESUMEN_VERSIONES.md` is a hand-curated historical table that still marks `v3.1.4` as "Actual" — it has not been updated in the bump and would need a dedicated entry with narrative. `git log` remains the authoritative source for the actual commit version.
