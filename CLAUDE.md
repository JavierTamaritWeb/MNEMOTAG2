...
**Current version: v3.3.17** (infrastructure release — AVIF EXIF parser ISOBMFF defensivo). Añade `_parseIsobmffBoxes(bytes)`, `_isAvifFile(bytes)`, `embedExifInAvifBlob(blob)` y `embedExifInAvifDataUrl(dataUrl)` a `MetadataManager`. El parser ISOBMFF maneja correctamente los headers compactos, large-size de 64 bits y size=0 (hasta el final). Detecta AVIF por major brand `avif` o por presencia en compatible_brands. **Importante limitación**: la inyección efectiva del item Exif (que requiere reescribir iinf, iref e iloc del meta box) NO está implementada — `embedExifInAvifBlob` siempre devuelve el blob original SIN tocarlo cuando todo el parseo es válido, y log un `console.warn` explicando que la inyección completa requiere una reescritura ISOBMFF. **NUNCA produce AVIF corruptos** — todos los caminos defensivos devuelven el blob original. Integrado en los 4 puntos del flujo de descarga existentes (igual que JPEG/PNG/WebP). 8 aserciones binarias nuevas validan el parser con un AVIF mínimo sintetizado a mano. Sources kept in sync: `index.html` `<title>`, `README.md` badge + NOVEDADES heading, `CHANGELOG.md` header + footer + topmost entry, and `docs/INDICE_DOCUMENTACION.md`. Note: `docs/RESUMEN_VERSIONES.md` is a hand-curated historical table that still marks `v3.1.4` as "Actual" — it has not been updated in the bump and would need a dedicated entry with narrative. `git log` remains the authoritative source for the actual commit version.
